...
def main(argc, argv):
    if argc < 2:
        print("ImportEmboxFB.py: Error: You must provide a Structured Text file with Embox Function Blocks")
        exit(-1)
    ...
    try:
        ...
        print(f"ImportEmboxFB.py: Error: File '{st_file_path}' not found")
        exit(-1)
    except IOError:
        print(f"ImportEmboxFB.py: Error: Could not read file '{st_file_path}'")
        exit(-1)
    ...
    xml_file_path = os.path.join(base_dir, 'plcopen', 'Embox_Function_Blocks.xml')
    ...
    try:
        ...
        print(f"ImportEmboxFB.py: Error: File '{xml_file_path}' not found")
        exit(-1)
    except IOError:
        print(f"ImportEmboxFB.py: Error: Could not write to file '{xml_file_path}'")
        exit(-1)

    for fb in function_blocks:
        print(f"ImportEmboxFB.py: Importing '{fb.name}'")
